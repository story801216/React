...
function App(props) {
  ...
  // checkbox - group複選複數核取方塊-用陣列來代表初始值
    ...
    <>
      ...
      <section id="checkboxGroup">
        ...
        <input
          type="checkbox"
          value="芒果"
          checked={likeList.includes('芒果')} // includes() 方法會判斷陣列是否包含特定的元素，並回傳布林值
          onChange={(e) => {
            // toggle(切換)
            // 如果這選項 在 陣列中 -> 移出陣列 (取消勾選)
            if (likeList.includes(e.target.value)) {
              //1. 拷貝新陣列
              //2. 新陣列中作處理
              // filter()會把陣列根據條件做篩選，並回傳一個新的陣列
              ...
              // 如果這選項 不在 陣列中 -> 加入陣列(勾選)
              //1. 拷貝新陣列
              //2. 新陣列中作處理
              const newLikeList = [...likeList, e.target.value] //複製一個新陣列，並將e.target.value直接設定進去
              //3. 設定回狀態
              setLikeList(newLikeList)
            }
          }}
